...
def download_data_in_chunks(breeze, start_date, end_date):
    ...
    # Breeze restricts to 1000 rows per API call for 1-min data.
    # To be extremely safe and avoid any data loss, we fetch 1 day at a time.
...
def download_options_for_all(breeze):
    for symbol in ["NIFTY", "CNXBAN"]:
        global SYMBOL
        SYMBOL = symbol
        download_data_in_chunks(breeze, START_DATE, END_DATE)

if __name__ == "__main__":
    API_SESSION = "55219280"
    breeze_client = setup_client()
    if breeze_client:
        download_options_for_all(breeze_client)
